...
from .costs import cycle_cost, execution_price, floor_step, realized_taker_fee, valid_quantity
from .data.funding_proxy import resolve_funding_mark
from .data.market_calendar import closure_for_minute
from .diagnostics import renewal_diagnostics, signal_diagnostics
from .events import DataGap, event_time, group_events
from .execution import eligible_trade, minute_capacity, minute_vwap, minute_window, observe_vwap
...
from .portfolio import hedge_error, joint_quantity, needs_rebalance, required_cash, target_quantity
...
class Backtest:
    def __init__(
        ...
        self.trades: dict[tuple, Trade | MinutePrice] = {}
        self.closed_bars: dict[tuple, MinuteBar] = {}
        ...
        self.renewal_diagnostics: list[dict] = []
    ...
    def _fresh(self, symbol: str) -> bool:
        if self._uses_closed_signals():
            spot = self.closed_bars.get((symbol, "spot"))
            future = self.closed_bars.get((symbol, "futures"))
            return bool(
                spot is not None
                and future is not None
                and spot.open_time == future.open_time
                and spot.end_time == future.end_time
                and 0 <= self.now - spot.available_at <= self.config.freshness_seconds * SECOND
                and spot.base_volume > 0
                and future.base_volume > 0
            )
        return all(
    ...
    def _observed_basis(self, symbol: str) -> D:
        ...
        return basis(spot.price, future.price)

    def _uses_closed_signals(self) -> bool:
        return (
            self.config.execution_model == "next_minute_vwap"
            or self.config.signal_price_model == "closed_minute"
        )

    def _signal_prices(self, symbol: str):
        observations = self.closed_bars if self._uses_closed_signals() else self.trades
        return observations.get((symbol, "spot")), observations.get((symbol, "futures"))

    def _joint_plan(self, symbol: str, equity_snapshot: D):
        p = self.ledger.positions[symbol]
        return joint_quantity(
            max(D(0), equity_snapshot),
            self.trades[(symbol, "spot")].price,
            self.trades[(symbol, "futures")].price,
            self.marks[symbol].close,
            p.spot,
            p.short,
            max(D(0), self._available(symbol)),
            self._rule(symbol, "spot"),
            self._rule(symbol, "futures"),
            self.config,
            existing_collateral=p.collateral,
            existing_average=p.average,
        )
    ...
    def _submit(self, symbol: str, market: str, side: str, quantity: D, purpose: str) -> bool:
        ...
        if self.config.execution_model == "next_minute_vwap":
            order.window_start, order.window_end = minute_window(self.now)
            order.deadline = order.window_end
    ...
    def _cancel(self, symbol: str, cause: str) -> None:
        for order in self._pending(symbol):
            if (
                self.config.execution_model == "next_minute_vwap"
                and order.window_start < self.now < order.window_end
            ):
                # A committed interval cannot be erased by a later observation.
                order.cancel_requested_at = self.now
                self.order_rows.append(self._order_row(order, f"deferred_cancel:{cause}"))
                continue
    ...
    def _finish_close(self, symbol: str) -> None:
        pair = self.pairs[symbol]
        if any(o.cancel_requested_at is not None for o in self._pending(symbol)):
            # A cancellation request cannot finalize a cycle while its already
            # committed interval may still create inventory at the boundary.
            if pair.state not in (State.CLOSING_PERP, State.CLOSING_SPOT, State.LIQUIDATING):
                self._transition(symbol, State.CLOSING_SPOT, "awaiting_committed_window")
            return
    ...
    def _risk(self, symbol: str, periodic: bool = False) -> None:
        ...
        if pair.state == State.HOLDING and needs_hedge_correction(p, self.config):
            if p.spot == 0:
                ...
                return
            pair.correction_deadline = (
                minute_window(self.now)[1]
                if self.config.execution_model == "next_minute_vwap"
                else self.now + self.config.correction_seconds * SECOND
            )
    ...
    def _timeout(self, order: Order) -> None:
        ...
        if order.purpose in ("close_perp", "liquidate", "close_spot"):
            ...
            if self.config.execution_model == "next_minute_vwap":
                p = self.ledger.positions[order.symbol]
                if hedge_error(p.spot, p.short) <= self.config.hedge_tolerance:
                    self.pairs[order.symbol].correction_deadline = None
                    if self.pairs[order.symbol].state != State.HOLDING:
                        self._transition(order.symbol, State.HOLDING, "hedge_corrected")
                else:
                    self._close(order.symbol, "hedge_correction_deadline")
            else:
                self._correct(order.symbol)
        elif order.purpose in ("increase_spot", "reduce_perp"):
            if self.config.execution_model == "next_minute_vwap" and order.filled_quantity:
                self._close(order.symbol, "partial_first_adjustment")
            else:
    ...
    def _entry(self, symbol: str, equity_snapshot: D) -> str:
        ...
        p = self.ledger.positions[symbol]
        rule = self._rule(symbol, "spot")
        if self.config.sizing_model == "joint_quantity":
            plan = self._joint_plan(symbol, equity_snapshot)
            if not plan.feasible:
                return (
                    "insufficient_free_funds"
                    if plan.reason == "insufficient_cash"
                    else "joint_sizing_infeasible"
                )
            if plan.spot_side != "buy" or plan.futures_side != "sell":
                return "no_positive_increment"
            quantity, needed = plan.spot_quantity, plan.required_cash
            pair.planned_short = plan.short_total
        else:
    ...
    def _renew(self, symbol: str, equity_snapshot: D) -> None:
        ...
        if self.config.sizing_model == "joint_quantity":
            plan = self._joint_plan(symbol, equity_snapshot)
            if not plan.feasible:
                self._event(symbol, "rebalance_skipped", cause=f"joint_sizing:{plan.reason}")
                return
            if plan.spot_side == "buy" and self.now < pair.cooldown_until:
                self._event(symbol, "rebalance_skipped", cause="cooldown")
                return
            pair.planned_short = plan.short_total
            pair.adjustment_quantity = plan.spot_quantity if plan.spot_side == "sell" else D(0)
            self.reservations[symbol] = plan.required_cash
            if plan.spot_side == "buy":
                sent = self._submit(symbol, "spot", "BUY", plan.spot_quantity, "increase_spot")
            elif plan.futures_side == "buy":
                sent = self._submit(symbol, "futures", "BUY", plan.futures_quantity, "reduce_perp")
            elif plan.spot_side == "sell":
                sent = self._submit(symbol, "spot", "SELL", plan.spot_quantity, "reduce_spot")
            elif plan.futures_side == "sell":
                sent = self._submit(
                    symbol, "futures", "SELL", plan.futures_quantity, "increase_perp"
                )
            else:
                sent = False
            if sent:
    ...
    def _next_leg(self, symbol: str) -> None:
        ...
        if purpose in ("open_perp", "increase_perp"):
            planned = pair.planned_short
            if self.config.sizing_model == "joint_quantity" and planned is not None:
                if hedge_error(p.spot, planned) > self.config.hedge_tolerance:
                    self._close(symbol, "actual_inventory_differs_from_joint_plan")
                    return
                quantity = planned - p.short
            else:
    ...
    def _after_fill(self, order: Order) -> None:
        ...
        leg_delay = (
            0
            if self.config.execution_model == "next_minute_vwap"
            else self.config.leg_delay_seconds * SECOND
        )
        if purpose in ("open_spot", "increase_spot"):
            pair.next_leg_at = self.now + leg_delay
    ...
    def _execute(self, order: Order, reference: D, trade_id: str, quantity: D | None = None) -> D:
        window_model = self.config.execution_model == "next_minute_vwap"
        quantity = order.remaining_quantity if quantity is None else quantity
        ...
        if window_model:
            p = self.ledger.positions[order.symbol]
            if order.market == "spot" and order.side == "SELL":
                quantity = min(quantity, floor_step(p.spot, rule.step))
            elif order.market == "futures" and order.side == "BUY":
                quantity = min(quantity, floor_step(p.short, rule.step))
            else:
                available = max(D(0), self._available(order.symbol))
                unit_cost = price
                if order.market == "futures":
                    unit_cost = (
                        price / self.config.leverage
                        + max(D(0), self.marks[order.symbol].close - price)
                        + price * rule.taker_fee * self.config.cost_multiplier
                    )
                quantity = min(quantity, floor_step(available / unit_cost, rule.step))
        if not valid_quantity(quantity, price, rule):
            return D(0)
        fill = Fill(
            f"fill-{order.order_id}" if not window_model else f"fill-{order.order_id}-{self.now}",
            order.order_id,
            order.symbol,
            order.market,
            order.side,
            ...
            trade_id,
            realized_taker_fee(
                rule, self.config, order.window_start if window_model else self.now
            ) * self.config.cost_multiplier,
        ...
        full = order.remaining_quantity == 0
        order.status = "filled" if full else "pending"
        self.order_rows.append(self._order_row(order, "filled" if full else "partial_fill"))
        ...
        row.update(
            ...
            slippage_rate=str(self.config.slippage * self.config.cost_multiplier),
        ...
        if full and order.cancel_requested_at is None:
            self._after_fill(order)
    ...
    def _fill_minute_windows(self, bars: list[MinuteBar]) -> None:
        """Settle committed windows before risk and expiration at this boundary."""
        indexed = {(b.symbol, b.market, b.open_time): b for b in bars}
        consumed = defaultdict(lambda: D(0))
        for order in list(self.orders.values()):
            if order.status != "pending" or order.window_end != self.now:
                continue
            key = (order.symbol, order.market, order.window_start)
            bar = indexed.get(key)
            if bar is None:
                if closure_for_minute(order.symbol, order.market, order.window_start) is None:
                    self._halt(
                        f"Missing execution minute for {order.symbol} {order.market}: {iso(order.window_start)}"
                    )
                    return
                continue
            reference = minute_vwap(bar)
            if reference is None:
                continue
            rule = self._rule(order.symbol, order.market)
            if rule is None:
                self._halt(f"Missing execution rules for {order.symbol}")
                return
            quantity = minute_capacity(
            ...
            if quantity > 0:
                filled = self._execute(order, reference, bar.reference_id, quantity)
                consumed[key] += filled
                if filled:
                    self.fills[-1].update(
                        window_base_volume=str(bar.base_volume),
                        window_quote_volume=str(bar.quote_volume),
                        participation=str(filled / bar.base_volume),
                        max_volume_participation=str(self.config.max_volume_participation),
                        capacity_used=str(consumed[key]),
                        fill_reason="full"
                        if order.remaining_quantity == 0
                        else "capacity_or_funds",
        ...
        for symbol in sorted(
            {
                ...
                if o.window_end == self.now and o.cancel_requested_at is not None
            }
        ):
            self._close(symbol, "cancelled_committed_window")

    def _check_minute_coverage(self, bars: list[MinuteBar]) -> bool:
        """Unknown observations are coverage failures, never market inactivity."""
        if self.now % (60 * SECOND):
            return True
        closed = {(b.symbol, b.market, b.end_time) for b in bars}
        for symbol in self.config.symbols:
            mark = self.marks.get(symbol)
            if mark is None or mark.available_at != self.now:
                self._halt(f"Missing risk mark minute for {symbol}: {iso(self.now)}")
                return False
            for market in ("spot", "futures"):
                if (symbol, market, self.now) not in closed and closure_for_minute(
                    symbol, market, self.now - 60 * SECOND
                ) is None:
                    self._halt(f"Missing price minute for {symbol} {market}: {iso(self.now)}")
                    return False
        return True
    ...
    def process(self, time_ns: int, records: list, native) -> None:
        ...
        trades, settlements, bars = [], [], []
        # Phase 1: all data at this timestamp, in stable symbol/event order.
        for r in records:
            minute_model = self.config.execution_model in ("minute_open", "next_minute_vwap")
            if (
                (minute_model and isinstance(r, Trade))
                or (not minute_model and isinstance(r, (MinutePrice, MinuteVolume, MinuteBar)))
                or (
                    self.config.execution_model == "next_minute_vwap"
                    and isinstance(r, (MinutePrice, MinuteVolume))
                )
                or (
                    self.config.execution_model == "minute_open"
                    and isinstance(r, MinuteBar)
                    and not self._uses_closed_signals()
                )
            ):
                self._halt("Price feed is incompatible with the configured execution model")
                return
            if isinstance(r, Trade):
                ...
                while (
                    ...
                    volume.popleft()
            elif isinstance(r, MinuteBar):
                self.closed_bars[(r.symbol, r.market)] = r
                bars.append(r)
                if self.config.execution_model == "next_minute_vwap":
                    if r.base_volume > 0:
                        self.trades[(r.symbol, r.market)] = r
                    volume = self.recent_volume[(r.symbol, r.market)]
                    volume.append((r.end_time - 1, r.base_volume))
                    while (
                        volume
                        and volume[0][0] < self.now - self.config.participation_seconds * SECOND
                    ):
                        volume.popleft()
            elif isinstance(r, MinutePrice):
                self.trades[(r.symbol, r.market)] = r
                trades.append(r)
            elif isinstance(r, MinuteVolume):
                volume = self.recent_volume[(r.symbol, r.market)]
                # Count a closed candle only once it has been published. Its close
                # timestamp keeps the submission-window upper bound exclusive.
                volume.append((r.close_time, r.quantity))
                while (
                    volume and volume[0][0] < self.now - self.config.participation_seconds * SECOND
                ):
                    volume.popleft()
        ...
        if self.config.execution_model == "next_minute_vwap" and not self._check_minute_coverage(
            bars
        ):
            return
        ...
        if self.config.execution_model == "next_minute_vwap":
            self._fill_minute_windows(bars)
        ...
        for symbol in due_renewals:
            diagnostic = renewal_diagnostics(self, symbol, snapshot)
            self._renew(symbol, snapshot)
            diagnostic.update(
                time_ns=self.now,
                strategy=self.strategy,
                symbol=symbol,
                state_after=self.pairs[symbol].state.value,
                expiry_after=self.pairs[symbol].expiry,
            )
            self.renewal_diagnostics.append(diagnostic)
        for symbol, fc in new_signals:
            diagnostic = signal_diagnostics(self, symbol, snapshot)
        ...
        # Phase 6: first eligible price; an order sent now cannot use this batch.
        for trade in trades:
            for order in list(self._pending(trade.symbol)):
                if eligible_trade(order, trade):
                    if self.config.execution_model == "vwap":
                        ...
                        reference_id = (
                            trade.reference_id if isinstance(trade, MinutePrice) else trade.trade_id
                        )
                        self._execute(order, trade.price, reference_id)
        if self.config.execution_model == "vwap":
            for order in list(self.orders.values()):
                if (
