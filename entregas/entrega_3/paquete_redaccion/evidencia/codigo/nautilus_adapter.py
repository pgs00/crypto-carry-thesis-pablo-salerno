...
class NautilusAdapter:
    ...
    def execute(self, fill: Fill) -> None:
        ...
        filled_before = Decimal(str(native.filled_qty))
        matcher.fill_order(
            native,
        ...
        if len(self.fill_events) != previous + 1:
            raise RuntimeError("Native fill event was not produced exactly once")
        filled_after = Decimal(str(native.filled_qty))
        if filled_after != filled_before + fill.quantity:
            raise RuntimeError("Native cumulative filled quantity diverged")
        complete = filled_after == Decimal(str(native.quantity))
        if native.is_closed != complete:
            raise RuntimeError("Native partial/full order state diverged")
