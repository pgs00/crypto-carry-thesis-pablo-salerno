...
def _write_table(run: Path, name: str, rows, manifest: dict, parquet: bool = False) -> None:
    ...
    defaults = list(PARQUET_COLUMNS[name] if parquet else CSV_COLUMNS[name])
    time_fields = (
        NS_FIELDS
        | {
            "start",
...
def _persist_tables(run, config, backtests, quality, manifest):
    tables = {name: [] for name in (*PARQUET_COLUMNS, *CSV_COLUMNS)}
    ...
    for name, rows in tables.items():
        _write_table(run, name, rows, manifest, parquet=name in PARQUET_COLUMNS)
...
def _report_text(run: Path) -> str:
    ...
    report = [
        ...
        (
            ...
            "Las diferencias entre regímenes no demuestran causalidad ni prueban la entrada de arbitrajistas."
        ),
        ...
        (
            ...
            "trade conocido y perpetuos el último mark disponible; no se mezcla un basis de trade con una valuación mark "
    ...
    report.extend(
        [
            ...
            (
                ...
                "ventana, vida media, horizonte, ejecución VWAP e inicios alternativos en sus propias corridas. "
...
def write_run(
    ...
    run = Path(root).resolve() / "outputs" / run_id
